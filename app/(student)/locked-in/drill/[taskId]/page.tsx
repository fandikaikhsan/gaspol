...
export default function DrillRunnerPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let moduleId = (taskData as { module_id?: string }).module_id
      ...
      if (moduleData) {
        ...
        const questionIds = (moduleData as { question_ids: string[] })
        ...
        if (questionsData) {
          const typedQuestionsData = questionsData as Question[]
          const sorted = questionIds
            .map((id) => typedQuestionsData.find((q) => q.id === id))
  ...
  const handleComplete = async (session: AssessmentSession) => {
    ...
    try {
      ...
      // Mark task complete
      await (supabase.from("plan_tasks") as any)
        .update({
          is_completed: true,
