    ...
    const fetchQuestions = async () => {
      ...
      try {
        ...
        if (moduleId) {
          ...
          const typedModuleData = moduleData as {
            name: string
            question_count: number
            question_ids: string[]
          }
          title = typedModuleData.name
          subtitle = `${typedModuleData.question_count} questions`

          const questionIds = typedModuleData.question_ids as string[]
          ...
          questionsData = shuffleArray((qs || []) as any[]).slice(
            0,
            questionCount,
          )
          ...
          title = `Focus: ${(node as { name: string } | null)?.name || "Topic"}`
