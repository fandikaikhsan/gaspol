...
export default function OnboardingPage() {
  ...
  const [campusResults, setCampusResults] = useState<
    Array<{
      id: string
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const { data } = await supabase
          ...
          .or(
            `university_name.ilike.%${campusQuery}%,major.ilike.%${campusQuery}%`,
          )
          .eq("verified", true)
  ...
  const handleComplete = async () => {
    ...
    if (!examDate || !daysRemaining || daysRemaining < 1) {
      toast({
        ...
        description: t("toast.examDateRequired", {
          fallback: "Please select a valid exam date.",
        }),
      })
      return
    }

    const effectiveTimeBudget = useCustomTime
      ? Number(customTimeBudget)
      : timeBudget
    if (
      !effectiveTimeBudget ||
      effectiveTimeBudget < 10 ||
      effectiveTimeBudget > 480
    ) {
      toast({
        variant: "destructive",
        title: t("toast.missingInfo", { fallback: "Missing Information" }),
        description: t("toast.timeBudgetInvalid", {
          fallback: "Please enter a valid study time (10–480 minutes).",
        }),
      })
      return
    }
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t("examDate.subtitle", {
                  fallback:
                    "Select your UTBK exam date. We'll create a study plan that counts down to the big day.",
                })}
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-2xl font-bold">
                        {daysRemaining}{" "}
                        {t("examDate.daysLeft", { fallback: "days remaining" })}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {daysRemaining < 7
                          ? t("examDate.urgentMsg", {
                              fallback:
                                "Time is short — we'll optimize for high-impact practice!",
                            })
                          : daysRemaining < 14
                            ? t("examDate.moderateMsg", {
                                fallback:
                                  "Good amount of time for focused preparation.",
                              })
                            : t("examDate.comfortMsg", {
                                fallback:
                                  "Great! Plenty of time to build a solid foundation.",
                              })}
                      </p>
                    </div>
                  </div>
        ...
        {step === 2 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("time.subtitle")}</CardDescription>
            </CardHeader>
                      ...
                      {useCustomTime && (
                        <div className="mt-2 flex items-center gap-2">
                          <Input
                            ...
                            onChange={(e) =>
                              setCustomTimeBudget(e.target.value)
                            }
                            placeholder="45"
                            className="w-24 h-9"
                          ...
                          <span className="text-sm text-muted-foreground">
                            {t("time.minutesPerDay", {
                              fallback: "minutes/day",
                            })}
                          </span>
                        </div>
                      )}
        ...
        {step === 3 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("target.subtitle")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder={t("target.universityPlaceholder", {
                      fallback: "Search university or major...",
                    })}
                    value={campusQuery}
                    onChange={(e) => {
                    ...
                    onFocus={() =>
                      campusResults.length > 0 && setShowCampusDropdown(true)
                    }
                    ...
                    {campusResults.map((campus) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setCampusQuery(
                            `${campus.university_name} — ${campus.major}`,
                          )
                ...
                {showCampusDropdown &&
                  ...
                  !isCampusSearching && (
                    <div className="border-2 border-border rounded-lg p-4 bg-muted/50 text-sm text-muted-foreground text-center">
                      {t("target.noResults", {
                        fallback:
                          "No campus found. You can still type your university name manually.",
                      })}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-surface-plan/30 border-2 border-primary/20">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-semibold">
                        {selectedCampus.university_name}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {selectedCampus.major} · Target score:{" "}
                        {selectedCampus.min_score}
                      </p>
                    </div>
              ...
              {!selectedCampus && (
                <div className="space-y-2">
                  <Label htmlFor="major">{t("target.majorLabel")}</Label>
                  <Input
                    id="major"
              ...
              <div className="bg-muted p-4 rounded-lg mt-6 space-y-2">
                ...
                <ul className="text-sm space-y-1">
                  <li>
                    📅 {t("summary.examDate", { fallback: "Exam date" })}:{" "}
                  ...
                  <li>
                    ⏱️{" "}
                    {t("summary.dailyStudy", {
                      minutes: useCustomTime
                        ? Number(customTimeBudget)
                        : timeBudget,
                    })}
                  </li>
                  ...
                  {selectedCampus && (
                    <li>
                      📊{" "}
                      {t("summary.targetScore", { fallback: "Target score" })}:{" "}
                      {selectedCampus.min_score}
                    </li>
                  )}
                </ul>
