...
describe('calculateAllConstructImpacts', () => {
  it('returns impacts for all weighted constructs', () => {
    const weights = { teliti: 0.3, speed: 0.7, reasoning: 0, computation: 0, reading: 0 }
...
describe('checkModulePassFail', () => {
  it('70% threshold: 7/10 → pass', () => {
    const result = checkModulePassFail(7, 10, 0.70)
    expect(result.passed).toBe(true)
    expect(result.scoreRatio).toBe(0.7)
  })

  it('70% threshold: 6/10 → fail', () => {
    const result = checkModulePassFail(6, 10, 0.70)
    expect(result.passed).toBe(false)
    expect(result.scoreRatio).toBe(0.6)
  })

  it('100% score → pass', () => {
    const result = checkModulePassFail(10, 10, 0.70)
    expect(result.passed).toBe(true)
    expect(result.scoreRatio).toBe(1)
  })

  it('0% score → fail', () => {
    const result = checkModulePassFail(0, 10, 0.70)
    expect(result.passed).toBe(false)
    expect(result.scoreRatio).toBe(0)
  })

  it('custom threshold: 80%', () => {
    expect(checkModulePassFail(8, 10, 0.80).passed).toBe(true)
    expect(checkModulePassFail(7, 10, 0.80).passed).toBe(false)
  })

  it('0 total questions → fail', () => {
    const result = checkModulePassFail(0, 0, 0.70)
    expect(result.passed).toBe(false)
    expect(result.scoreRatio).toBe(0)
  })
...
describe('deriveErrorTags', () => {
  ...
  it('incorrect with reading-heavy construct → ERR.READING', () => {
    const weights = { teliti: 0.1, speed: 0.1, reasoning: 0.1, computation: 0.1, reading: 0.8 }
  ...
  it('incorrect with computation-heavy construct → ERR.COMPUTATION', () => {
    const weights = { teliti: 0.1, speed: 0, reasoning: 0.2, computation: 0.5, reading: 0.1 }
    const tags = deriveErrorTags(false, 100, 100, weights)
  ...
  it('construct weight below 0.3 threshold → no construct tag', () => {
    const weights = { teliti: 0.2, speed: 0.2, reasoning: 0.2, computation: 0.2, reading: 0.2 }
    const tags = deriveErrorTags(false, 100, 100, weights)
    const constructTags = tags.filter(t =>
      ['ERR.ATTENTION', 'ERR.SPEED', 'ERR.REASONING', 'ERR.COMPUTATION', 'ERR.READING'].includes(t.tag_id)
    )
