...
function stemPreview(stem: string, maxLen = 220): string {
  const stripped = stem
    .replace(/\$\$[\s\S]*?\$\$/g, "[math]")
    .replace(/\$[^$\n]+\$/g, "[math]")
    .replace(/\s+/g, " ")
...
export interface DrillTutorQuestionModalProps {
  open: boolean
...
export function DrillTutorQuestionModal({
  open,
  onOpenChange,
  questionStem,
  questionNumber,
  ...
  /** First outbound message after open (or after question change) includes stem context for the model. */
  const includeStemContextRef = useRef(true)
  ...
  useEffect(() => {
    ...
    includeStemContextRef.current = true
    setMainInput("")
  ...
  const contextLine = t("question.tutorContextLine", {
    current: questionNumber,
    total: totalQuestions,
    module: moduleLabel,
    preview: stemPreview(questionStem),
  })

  const sendMessage = useCallback(async () => {
    const trimmedMain = mainInput.trim()
    const composed = includeStemContextRef.current
      ? [contextLine, trimmedMain].filter(Boolean).join("\n\n")
      : trimmedMain
    ...
    try {
      ...
      includeStemContextRef.current = false
      ...
      setMessages((prev) => [
        ...prev.filter((m) => m.id !== userMsg.id),
        {
          id: `err-${Date.now()}`,
          role: "assistant",
          message: t("question.tutorSendError"),
          created_at: new Date().toISOString(),
        },
      ])
    } finally {
  ...
  }, [contextLine, isLoading, isQuotaExhausted, mainInput, t])
      ...
      <DialogContent className="flex max-h-[min(90dvh,720px)] w-[calc(100vw-1.25rem)] max-w-[520px] flex-col gap-0 overflow-hidden rounded-xl border-2 border-black bg-[#F5F5F0] p-0 shadow-[8px_8px_0px_0px_#000] sm:max-w-xl">
        ...
        <div className="space-y-2 border-t-2 border-black bg-[#F5F5F0] p-3">
          ...
          <Textarea
            ...
            placeholder={t("question.tutorModalPlaceholder")}
